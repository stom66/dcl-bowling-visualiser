import { countSimulationKeyframes, estimateSimulationBytes, estimateTrackBytes } from './keyframes'
import type { ChannelKey } from './keyframes'
...
export function buildSimulationChartModel(comparison: SimulationComparison): SimulationChartModel {
  ...
  const originalBytes = estimateSimulationBytes(comparison.original)
  const compressedBytes = estimateSimulationBytes(comparison.compressed)
...
function buildEntityStats(track: {
  ...
  const originalBytes = estimateTrackBytes(track.original)
  const compressedBytes = estimateTrackBytes(track.compressed)
